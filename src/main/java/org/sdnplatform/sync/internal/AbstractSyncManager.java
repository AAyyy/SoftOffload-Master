 ...
 * @author readams
 */
public abstract class AbstractSyncManager implements ISyncService,
		IFloodlightModule {
	...
	public <K, V> IStoreClient<K, V> getStoreClient(String storeName,
		...
		return getStoreClient(storeName, keyClass, null, valueClass, null, null);
	}

	@Override
	public <K, V> IStoreClient<K, V> getStoreClient(String storeName,
			TypeReference<K> keyType, TypeReference<V> valueType)
			throws UnknownStoreException {
		return getStoreClient(storeName, null, keyType, null, valueType, null);
	}
	...
	public <K, V> IStoreClient<K, V> getStoreClient(String storeName,
			...
			throws UnknownStoreException {
		return getStoreClient(storeName, null, keyType, null, valueType,
				resolver);
	}
	...
	public <K, V> IStoreClient<K, V> getStoreClient(String storeName,
			...
			IInconsistencyResolver<Versioned<V>> resolver)
			throws UnknownStoreException {
		return getStoreClient(storeName, keyClass, null, valueClass, null,
				resolver);
	}
	...
	/**
	 * The "real" version of getStoreClient that will be called by all the
	 * others
	 * 
	 * @param storeName
	 *            the store name
	 ...
	 */
	public <K, V> IStoreClient<K, V> getStoreClient(String storeName,
			Class<K> keyClass, TypeReference<K> keyType, Class<V> valueClass,
		...
		if (valueType != null && keyType != null) {
			serializingStore = new JacksonStore<K, V>(store, keyType, valueType);
		} else if (valueClass != null && keyClass != null) {
			serializingStore = new JacksonStore<K, V>(store, keyClass,
					valueClass);
		} else {
		...
		DefaultStoreClient<K, V> storeClient = new DefaultStoreClient<K, V>(
				serializingStore, resolver, this, keyClass, keyType);
		return storeClient;
	}
	...
	public abstract void addListener(String storeName,
			MappingStoreListener listener) throws UnknownStoreException;
